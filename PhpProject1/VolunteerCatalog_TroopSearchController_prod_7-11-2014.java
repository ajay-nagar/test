public with sharing class VolunteerCatalog_TroopSearchController extends SobjectExtension {
  public String serachHeading { get; set;}
    ...
    public VolunteerCatalog_TroopSearchController() {
        selectedPageSize = '10';
        ...
        try {
            ...
            if(newTempWrapperList != null && newTempWrapperList.size() > 0) {
                ...
                if(parentCampaignWrapperList == null || parentCampaignWrapperList.size() == 0) {
                   return addErrorMessage('No Troop with this name exists');
                }
            }
        ...
        try {
            if(selectedPageSize != null && selectedPageNumber != null && selectedPageSize != '' && selectedPageNumber != '') {
                ...
                if(allParentCampaignWrapperList != null && allParentCampaignWrapperList.size() > 0) {
                    ...
                    return addErrorMessage('No results to display');
                }
            }
                    ...
                    for(ParentCampaignWrapper wrapper : parentCampaignWrapperList) {
                        if(wrapper.isCampaignChecked) {
                            ...
                            if(wrapper.parentCampaignAccountId != null) {
                                ...
                                addErrorMessage('Please contact the council for help with the \'' + wrapper.parentCampaignName + '\' role. Thank You.');
                            }
                        }
                    }
                }
                ...
                if(campaignMemberSaveresultList != null) {
                    ...
                    for (Database.Saveresult sr : campaignMemberSaveresultList) {
                        if (sr.isSuccess()) {
                            ...
                            Pagereference JoinMembershipInformationPage = new Pagereference('/apex/Volunteer_JoinMembershipInformation');
                            if(contactId != null && contactId != '')
   ...
   /* public Pagereference createCampaignMemberOnUnsureCheck() {
    ...
    public List<ParentCampaignWrapper> obtainParentCampaignWrapperList() {
        ...
        if(troopOrGroupName != null &&  troopOrGroupName != '' && zipCode != null && zipCode != '') {
            ...
            //List<Campaign> allCampaignList = VolunteerRegistrationUtilty.getListOfAllCampaign(troopOrGroupName,'TroopName');
            if(allCampaignList != null && allCampaignList.size() > 0) {
                searchByCampaignName = false;
                for(Campaign campaign : allCampaignList)
                    if(campaign != null && campaign.Id != null)
                        innerParentCampaignWrapperList.add(new ParentCampaignWrapper('0', campaign.Name, campaign.Parent.Grade__c, campaign.Parent.Meeting_Location__c,  campaign.Parent.Meeting_Day_s__c,campaign.Parent.Meeting_Frequency__c, campaign.Parent.Troop_Start_Date__c, campaign.Parent.Meeting_Start_Time__c, String.valueOf(campaign.Volunteer_Openings_Remaining__c), campaign.Parent.Name, campaign.Parent.Account__c, campaign.Id));
            ...
            for(Campaign campaign : allChildCampaignWithZipCodeList) {
                if(campaign != null && campaign.Id != null) {
                    if(parentCampaignIdVSDistanceMap.ContainsKey(campaign.Parent.Id)) {
                        tempParentCampaignWrapperList.add(new ParentCampaignWrapper(parentCampaignIdVSDistanceMap.get(campaign.Parent.Id), campaign.Name, campaign.Parent.Grade__c, campaign.Parent.Meeting_Location__c,  campaign.Parent.Meeting_Day_s__c,campaign.Parent.Meeting_Frequency__c, campaign.Parent.Troop_Start_Date__c, campaign.Parent.Meeting_Start_Time__c, String.valueOf(campaign.Volunteer_Openings_Remaining__c), campaign.Parent.Name, campaign.Parent.Account__c, campaign.Id));
                    }
                }
            }
        }
    ...
    public void addUnsureCampaign() {
        ...
        if(parentCampaignRecordList != null && parentCampaignRecordList.size() > 0) {
            ...
            if(childCampaign != null)
                unsureCampaignRecordList.add(new ParentCampaignWrapper('0', childCampaign.Name, childCampaign.Parent.Grade__c, childCampaign.Parent.Meeting_Location__c,  childCampaign.Parent.Meeting_Day_s__c, childCampaign.Parent.Meeting_Frequency__c, childCampaign.Parent.Troop_Start_Date__c, childCampaign.Parent.Meeting_Start_Time__c, String.valueOf(childCampaign.Volunteer_Openings_Remaining__c), childCampaign.Parent.Name, childCampaign.Parent.Account__c, childCampaign.Id));
        }
    }
